...
export default function rematra() {
  ...
  const [courierStep, setCourierStep] = useState<"selection" | "payment">("selection")
  ...
  // --- UPLOAD KE STORAGE (Bukan Base64 lagi) ---
  ...
  const handlePublish = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (p) => {
      ...
      // 3. Simpan ke Database (Hanya URL-nya saja, jadi ringan!)
      ...
      else { fetchMaterials(); alert("ALHAMDULILLAH TERBIT!"); setRole("dashboard"); setPhotoFile(null); setPhotoPreview(null); }
      setLoading(false)
    }, () => alert("GPS Wajib Aktif!"), { timeout: 5000 })
  }

  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-20 font-sans">
      
      {/* HEADER MEWAH (DIKUNCI) */}
      <header style={{ backgroundColor: '#EE4D2D' }} className="text-white py-12 px-6 text-center shadow-2xl rounded-b-[4rem] border-b-8 border-black/10">
        <img src="/Logo.jpeg" alt="Logo" className="h-20 mx-auto mb-4 rounded-3xl border-4 border-white/20 shadow-2xl" onClick={() => setRole("home")} />
        <h1 className="text-5xl font-black italic tracking-tighter uppercase">GO-MATERIAL</h1>
      ...
      <main className="max-w-5xl mx-auto p-6 mt-[-2rem]">
        ...
        {/* KATALOG PEMBELI (FILTER RADIUS 15KM AKTIF) */}
        {role === "buyer" && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {materials
                ...
                .filter(m => {
                    ...
                    return d <= 15 // --- KUNCI RADIUS 15 KM ---
                })
                .map(item => {
                    ...
                    <div key={item.id} className="bg-white rounded-[3.5rem] overflow-hidden shadow-2xl border border-gray-100 flex flex-col transition-transform hover:scale-[1.02]">
                      ...
                      <div className="p-8">
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => setSelectedItem(item)} className="flex-[3] bg-[#EE4D2D] text-white py-5 rounded-[2rem] font-black text-sm shadow-xl active:scale-95">BELI VIA ESCROW</button>
        ...
        {role === "seller" && (
          <div className="max-w-md mx-auto bg-white p-10 rounded-[4rem] shadow-2xl border-t-[12px] border-[#EE4D2D] space-y-5">
            ...
            <div className="border-4 border-dashed border-gray-100 p-2 rounded-[2.5rem] h-48 flex items-center justify-center overflow-hidden bg-gray-50 relative">
              {photoPreview ? <img src={photoPreview} className="w-full h-full object-cover" /> : <p className="text-gray-300 font-black italic text-xs">FOTO BARANG</p>}
            </div>
        ...
        {role === "home" && (
           <div className="text-center py-24 space-y-12 animate-in zoom-in duration-1000">
             <div className="space-y-3 px-6">
                ...
                <p className="text-xs font-black text-gray-400 uppercase tracking-[0.3em]">Arsitek.Sign x Go-Material</p>
             </div>
